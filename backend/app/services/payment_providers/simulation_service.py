...
import logging
from bson import ObjectId  # ✅ AJOUTÉ
...
class SimulationService:
    ...
    async def _auto_complete_payment(
        ...
        try:
            ...
            if payment:
                ...
                # ✅ FIX:  Convert string order_id to ObjectId for MongoDB query
                try:
                    order_object_id = ObjectId(order_id) if isinstance(order_id, str) else order_id
                except Exception as e:
                    logger.error(f"[SIMULATION] Invalid order_id format: {order_id} - {str(e)}")
                    return

                # Update associated order status
                order_result = await db.orders.update_one(
                    {"_id": order_object_id},  # ✅ CORRECTION ICI
                    {
                        "$set": {
                            "status": "confirmed",
                ...
                # ✅ AMÉLIORATION:  Log détaillé du résultat
                if order_result.modified_count > 0:
                    logger. info(f"[SIMULATION] ✅ Auto-completed payment {payment_id} and updated order {order_id}")
                else:
                    logger.warning(f"[SIMULATION] ⚠️ Payment {payment_id} completed but order {order_id} was not updated (matched:  {order_result.matched_count})")

            else:
                logger.warning(f"[SIMULATION] Payment {payment_id} not found after completion")

        except Exception as e: 
            logger.error(f"[SIMULATION] Error auto-completing payment {payment_id}: {str(e)}", exc_info=True)  # ✅ Ajout exc_info
    ...
    async def _auto_fail_payment(
        ...
        try:
            ...
            logger.error(f"[SIMULATION] Error auto-failing payment {payment_id}: {str(e)}", exc_info=True)  # ✅ Ajout exc_info
